...
async function APIEnsembl (rs, popu, size) {
  try {
    ...
    for (let i = 0; i < rs.length; i++) {
      ...
      if (error === 400) {
        console.log(error)
        linkGtex = 'https://www.gtexportal.org/home/snp/' + rs[i]
      }
      let result = {
        name: acces38.name,
...
function writeFile (wstream, tab) {
  if (tab) {
    for (let i = 0; i < tab.length; i++) {
      let txt = 'Ensembl Information : \r\n' +
          'Name : ' + tab[i].name + ',\r\n' +
          'Consequence : ' + tab[i].conseq + ',\r\n' +
          'Allele : ' + tab[i].allele + ',\r\n' +
          'MAF : ' + tab[i].maf + ',\r\n' +
          'Ancestral : ' + tab[i].ancestral + ',\r\n' +
          'Location (38) : ' + tab[i].location + ',\r\n' +
          'Location (37) : ' + tab[i].location2 + ',\r\n' +
          'VCF \r\n' +
          'Start : ' + tab[i].start + ',\r\n' +
          'End : ' + tab[i].end + ',\r\n' +
          'Sequence_Region -> Chromosome : ' + tab[i].seq + ',\r\n' +
          'Clinical Significance : ' + tab[i].clinicSigni + ',\r\n'
      let variant = 'Variant : '
      for (let j in tab[i].variant) {
        let foo = tab[i].variant[j] + ','
        variant = variant + foo
      }
      let num = variant.length - 1
      variant = variant.substr(0, num)
      txt = txt + variant + '\r\n' +
        'GTEX Information : \r\n' +
        'Genecode Id        Gene Symbol     P-Value                      Tissue\r\n'
      for (let h in tab[i].gtexvalue) {
        let tmp = tab[i].gtexvalue[h].geneid + '        ' + tab[i].gtexvalue[h].symbol + '         ' + tab[i].gtexvalue[h].pvalue + '          ' + tab[i].gtexvalue[h].tissu + '\r\n'
        txt = txt + tmp
      }
      txt = txt + 'GWAS Information : \r\n' +
        'Name        RiskFrequency    P-Value          Range           Or   Traits          Study\r\n'
      for (let k in tab[i].gwasvalue) {
        let tp = tab[i].gwasvalue[k].name + '        ' + tab[i].gwasvalue[k].riskFrequency + '        ' + tab[i].gwasvalue[k].pvalue + '         ' + tab[i].gwasvalue[k].range + '   ' + tab[i].gwasvalue[k].or + '   ' + tab[i].gwasvalue[k].trait.listname[0] + '          ' + tab[i].gwasvalue[k].study.pubmedId + '\r\n'
        txt = txt + tp
      }
      txt = txt + '\r\n' + '------------------------------------------------------------------------------------------------------------------------' + '\r\n' + '\r\n' + '\r\n'
      wstream.write(txt)
