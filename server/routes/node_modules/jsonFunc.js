...
async function searchRs (rs, res) {
  ...
  const val = gtexApi(rs, geneList, gt.body.mappings[0].minor_allele, gt.body.mappings[0].ancestral_allele)
  let result = {
    name: gt.body.name,
...
// Function de recherche GTEX API
async function gtexApi (rs, geneList, minor, ancestral) {
  let results = []
  for (let i in geneList) {
    if (geneList[i] !== 'NAN') {
      console.log(geneList[i])
      const id = geneList[i]
      // const gtsearch = await superagent.get(gt + id).set('Accept', 'application/json')
      const gtsearch = await superagent.get('rest.ensembl.org/lookup/id/ENSG00000139344').set('Accept', 'application/json')
      console.log('Resultat : ' + gtsearch.body.source)
