  ...
  try {
    ...
    for (let i = 0; i < rs.length; i++) {
      ...
      for (let x in gwAsso.body._embedded.associations) {
        if (typeof (gwAsso.body._embedded.associations[0]) !== 'undefined') {
      ...
      let geneids = []
      let geneList = []
      for (let k in gw.body.genomicContexts) {
        if (typeof (gw.body.genomicContexts[k].gene.ensemblGeneIds[0]) !== 'undefined') {
          geneList.push(gw.body.genomicContexts[k].gene.ensemblGeneIds[0].ensemblGeneId)
        } else {
          geneList.push('NAN')
        }
      }
      for (let j in geneList) {
        if (geneList[j] !== 'NAN') {
          const id = geneList[j]
          // voir si il faut utiliser le 37 ou  le 38 , et mettre en place accès au deux
          // const gtsearch = await superagent.get(ensembl37 + 'lookup/id/' + id).set('Accept', 'application/json')
          const gtsearch = await superagent.get(ensembl38 + 'lookup/id/' + id).set('Accept', 'application/json')
          if (typeof gtsearch.body !== 'undefined') {
            const variants = gtsearch.body.seq_region_name + '_' + gtsearch.body.end + '_' + acces38.minor_allele + '_' + acces38.ancestral_allele + '_b37'
            let resultat = {
              geneid: id,
              symbol: gtsearch.body.display_name,
              variantid: variants
      ...
      // Test: rs1042779; population: Vietnam; size: 500
      let adr = ensembl38 + 'ld/human/' + rs + '/' + popu + '?window_size=' + size
      const rcarre = await superagent.get(adr).set('Accept', 'application/json')
      let listr2 = []
      for (let i in rcarre.body) {
        let rd = {
          name: rcarre.body[i].variation2,
          r2: rcarre.body[i].r2
        }
        listr2.push(rd)
      }
...
async function searchSnpByLocation (location) {
  try {
    let tab = []
    let snpList = []
    ...
    for (let i = 0; i < location.length; i++) {
      if (location[i]) {
        ...
        for (let j in gw.body._embedded.singleNucleotidePolymorphisms) {
          snpList.push(gw.body._embedded.singleNucleotidePolymorphisms[j].rsId)
          // console.log('boucle tour : ' + i + j)
        }
        let result = {
          snp: snpList
        }
...
/* async function searchR2 (rs, popu, size) {
  try {
    let adr = ensembl38 + 'ld/human/' + rs + '/' + popu + '?window_size=' + size
    const r2 = await superagent.get(adr).set('Accept', 'application/json')
    let listr2 = []
    for (let i in r2.body) {
      let rd = {
        name: r2.body[i].variantion2,
        r2: r2.body[i].r2
      }
      listr2.push(rd)
    }
...
// module.exports.searchR2 = searchR2
