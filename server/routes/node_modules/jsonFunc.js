...
async function APIEnsembl (rs) {
  try {
    ...
    for (let i = 0; i < rs.length; i++) {
      ...
      const gwAsso = await superagent.get('www.ebi.ac.uk/gwas/beta/rest/api/associations/search/findByRsId?rsId=' + rs).set('Accept', 'application/json')
      // const r2 = await superagent.get(ensembl38 + 'Id/human/' + rs[i]).set('Accept', 'application/json')
      let listAsso = []
      let listInfo = []
      let listSyn = []
...
async function searchSnpByLocation (location, nbmax) {
  try {
    let tab = []
    let snpList = []
    console.log('nombre max jsonFunc ' + nbmax)
